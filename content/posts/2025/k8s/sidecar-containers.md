...
En el post vamos a hablar del patrón `sidecar container`. El objetivo de este patrón es que uno o varios contenedores extiendan la funcionalidad principal de otro contenedor. 

Imagina que necesitas extraer parte de la información generada por tu aplicación. En vez de incluir esta funcionalidad dentro de tu aplicación haciéndola más pesada, delegamos esta trabajo en otra aplicación que es desplegada junto a tu aplicación leyendo y extrayendo la información generada.

Desde la versión de [Kubernetes 1.28 Planternetes (alpha)](https://Kubernetes.io/blog/2023/08/15/Kubernetes-v1-28-release/#sidecar-init-containers) soporta la configuración de este patrón de manera nativa y desde la versión [1.33 Octarine (stable)](https://Kubernetes.io/blog/2025/04/23/Kubernetes-v1-33-release/#stable-sidecar-containers). 

A continuación mostramos como podemos utilizarlo.
...
Esta manera de utilizar `sidecar containers` en Kubernetes presenta algunos problemas en algunos casos:
...
Definiendo el valor `Always` en la propiedad `restartPolicy` el contenedor se ejecutará durante todo el ciclo de vida del Pod y nos garantizará el orden correcto de ejecución de cada uno de los contenedores. Siendo los contenedores definidos en `initContainers` los primeros en ejecutarse.

Cuando se inicia el proceso de finalización del Pod, `kubelet` pospone la finalización de los contenedores definidos en `initContainers` hasta que los contenedores definidos en la proƒ√piedad `containers` hayan finalizado garantizando hasta el final del ciclo de vida del Pod el funcionamiento de los `sidecar containers`.

## Conclusión
Aunque la implementación mediante `initContainers` y la propiedad `restartPolicy` puede resultar menos intuitiva para definir los `sidecar containers` de nuestro Pod, creo que tener soporte nativo en Kubernetes simplifica significativamente la implementación de este patrón, resolviendo así los problemas de orquestación entre los contenedores y garantizando su correcto funcionamiento.
