...
summary: >
    Como funciona Vertical Pod Autoscaler en Kubernetes
date: "2023-05-18"
...
- [Recommender](https://github.com/kubernetes/autoscaler/blob/master/vertical-pod-autoscaler/pkg/recommender/README.md): es el componente principal del VPA, cuya misión es generar recomendaciónes de asignación de CPU y memoria basados en el histórico y consumo actual de CPU y memoria.
...
- [Admission Controller](https://github.com/kubernetes/autoscaler/blob/master/vertical-pod-autoscaler/pkg/admission-controller/README.md): por cada Podcreado en el clúster, comprueba si debe realizar una actualización de los recursos asignados basándose en la configuración del VPA (si la hay) y en los datos del `recommender`.
...
### Objecto VPA (Vertical Pod Autocaler)
Para poder definir el comportamiento del VPA para nuestros deplomentes es necesario crear un recurso de tipo `VerticalPodAutoscaler`:
...
- **containerPolicies**:
  - **containerName**: Nombre del contenedor (usar `*` para aplicar la configuración a todos los containers del pod)
  - **minAllowed**: Los recursos minimos que el VPA puede asignar al container (CPU y/o memoria)
  - **maxAllowed**: Los recuros máximos que el VPA puede asignar al container (CPU y/o memoria)
  - **controlledResources**: El tipo de recursos o recusos a monitorizar (CPU y/o memoria)
- **updatePolicy**: VPA ofrece cuatro modos para aplicar las recomendaciónes de asignación de CPU y memoria:
  - **Off**: El __recomender__ calcula las recomendaciónes de asignacion de recursos pero el __updater__ nunca reinica los Pods para aplicar los cambios (modo "dry-run")
  - **Initial**: Solo se se asignan las recomendaciónes cuando se crea el pod, no se realiza ningún cambio durante el ciclo de vida del mismo
  - **Recreate**: El VPA puede asignar las recomendaciónes cuando se crea el Pody durante su ciclo de vida
  - **Auto**: Actulamente equivale a la opción `Recreate` ya que es el unico metodo de actualización disponible (valor por defecto)


### Instalación
Acualmente VPA no está incluido como un componente en Kubernetes por lo que es necesario instalarlo desde su repositorio. 
...
1. Clonar repositorio https://github.com/kubernetes/autoscaler/tree/master
2. Ejecutamos el comando `./hack/vpa-up.sh` desde la carpeta `vertical-pod-autoscaler`

#### Azure (Azure Kubernetes Service)
Azure permite habilitar/deshabilitar Vertical Pod Autoscaler en un clúster AKS. Para ello, debemos registrar el "feature flag" `AKS-VPAPreview` utilizando el siguiente comando: `az feature register --namespace "Microsoft.ContainerService" --name "AKS-VPAPreview"`.
...
[![Muestra los 3 Pods ejecutandose en el cluster](/2023/kubernetes/vertical-pod-autoscaler-vpa-instalacion-ok.png)](/2023/kubernetes/vertical-pod-autoscaler-vpa-instalacion-ok.png)
...
[![Diagrama funcionamiento VPA](/2023/kubernetes/vertical-pod-autoscaler-all.png)](/2023/kubernetes/vertical-pod-autoscaler-all.png)
...
#### Logs recomender
Cada 1 minuto, el `recomender` comprueba las métricas y calcula si es necesario realizar una actualización de los recursos:
...
#### Logs updater
En el caso de haya una recomendación por parte del `recomender`, el `updater` eliminará los Pods asociados al deployment (razón: `EvictedByVPA` ). Este proceso también se ejecuta cada 1 minuto.
...
- Cada vez que se actualizán los recursos, los Pods son recreados.
- VPA no garantiza que una vez eliminados los Pods estos se puedan recrear (ej. no hay espacio suficiente en el nodepool).
- La escalabilidad vertican no debe utilizarse junto con la escalabilidad horizontal
- Antes de utilizar VPA, debemos comprobar los Admission controllers exitentes
